        ...
        <div className="p-3 bg-gray-100 rounded-lg">
            <div className='h-[65vh] overflow-scroll scrollbar-hide'>
                {frameList?.map((frame, index) => (
                    <div key={index} className={`flex flex-col items-center border-b p-2 m-2 hover:bg-blue-200 hover:rounded-2xl hover:cursor-pointer ${selectedFrame == index && 'bg-blue-200 rounded-2xl'}`} onClick={()=>setSelectedFrame(index)}>
                        <Image src={frame.image} alt={`Frame ${index}`} width={40} height={40} className='w-full h-[40px]object-contain rounded-lg mt-2' />
                        <h2 className='text-xs line-clamp-2 mt-1'>{frame.text}</h2>
                        {selectedFrame == index && <Trash2 className='mt-1 h-5 w-5 text-red-500' onClick={()=>removeFrame(index)}/>}
                    </div>
                ))}
